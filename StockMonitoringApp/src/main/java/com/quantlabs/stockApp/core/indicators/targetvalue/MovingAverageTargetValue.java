...
/**
 * Custom indicator for calculating target value based on two moving averages
 * and trend direction Target = MA1 + (MA1 - MA2) for uptrend Target = MA1 -
 * (MA1 - MA2) for downtrend
 */
public class MovingAverageTargetValue extends CachedIndicator<Num> {
	...
	public MovingAverageTargetValue(BarSeries series, String trendDirection, int ma1Period, int ma2Period,
			String ma1PriceType, String ma2PriceType, String ma1Type, String ma2Type) {
		super(series);
		this.series = series;
	...
	protected Num calculate(int index) {
		try {
			...
			if (ma1Value.isGreaterThan(ma2Value)) {
				difference = ma1Value.minus(ma2Value);
			}else {
				difference = ma2Value.minus(ma1Value);
			}
			Num targetValue;
	...
	public String getMa1PriceType() {
		return ma1PriceType;
	}

	public String getMa2PriceType() {
		return ma2PriceType;
	}

	public String getMa1Type() {
		return ma1Type;
	}

	public String getMa2Type() {
		return ma2Type;
	}
	...
	public String getConfiguration() {
		return String.format("MA1: %s%d(%s) | MA2: %s%d(%s) | Trend: %s", ma1Type, ma1Period, ma1PriceType, ma2Type,
				ma2Period, ma2PriceType, trendDirection);
	}
}