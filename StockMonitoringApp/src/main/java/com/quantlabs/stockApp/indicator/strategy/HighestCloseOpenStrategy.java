...
public class HighestCloseOpenStrategy extends AbstractIndicatorStrategy {
	...
	public HighestCloseOpenStrategy(String timerange, String session, int indexCounter, ConsoleLogger logger) {
		super(logger);
	...
	public String getName() {
		if(timeRange == "1D" && session == "all" && indexCounter == 5) {
			return "HIGHESTCLOSEOPEN";
		}else {
			return "HIGHESTCLOSEOPEN(" + timeRange +","+session+"," +indexCounter+")";
		}
	}
	...
	public void calculate(BarSeries series, AnalysisResult result, int endIndex) {
		...
		double value = 0;
		String status = "N/A (no data)";
		String name = getName();
		
		if (sessionEndIndex != -1) {
			try {
				// Get the highest value using the helper method
				HighestCloseOpenIndicator.HighestValue highestVal = HighestCloseOpenStrategy.getHighestCloseOpenValue(
						series, timeRange, session, currentDataSource, sessionEndIndex, indexCounter,
						currentAnalysisIndex);

				if (highestVal != null && highestVal.value != null) {
					// Now check for higher prices using indexCounter
					boolean hasHigherPrice = HighestCloseOpenStrategy.checkForHigherPriceInLastNBars(series, highestVal,
							indexCounter, currentAnalysisIndex);
					...
					value = highestVal.value.doubleValue();
					status = String.format("%s (%.2f @ %s) [Lookback: %d]", isUptrend ? "Uptrend" : "Downtrend",
							highestVal.value.doubleValue(), timeStr, indexCounter);
							
					//result.setHighestCloseOpen(highestVal.value.doubleValue());
					//result.setHighestCloseOpenStatus(	String.format("%s (%.2f @ %s) [Lookback: %d]", isUptrend ? "Uptrend" : "Downtrend",	highestVal.value.doubleValue(), timeStr, indexCounter));
				...
				// logToConsole("Error calculating HighestCloseOpen for " + symbol + " " + tf +
				// ": " + e.getMessage());
				//result.setHighestCloseOpen(0);
		...
		if (timeRange == "1D" && session == "all" && indexCounter == 5 ) {
			result.setHighestCloseOpen(value);
			result.setHighestCloseOpenZscore(zscore);
			result.setHighestCloseOpenStatus(status);
		} else {
			// save result in custom
			result.setCustomIndicatorValue(name + "_HIGHESTCLOSEOPEN", String.valueOf(value));
			result.setCustomIndicatorValue(name + "_HIGHESTCLOSEOPENTrend", String.valueOf(status));
			result.setCustomIndicatorValue(name + "_HIGHESTCLOSEOPENZscore", String.valueOf(zscore));
		}
		
	}
	
	public Double getHighestCloseOpen(AnalysisResult analysisResult) {
		try {
		if (timeRange == "1D" && session == "all" && indexCounter == 5 ) {
			return analysisResult.getHighestCloseOpen();
		} else {
			// save result in custom
			return Double.valueOf(analysisResult.getCustomIndicatorValue(getName() + "_HIGHESTCLOSEOPEN"));			
		}
		}catch(Exception e) {
			return 0.0;
		}
	...
	public String getHighestCloseOpenStatus(AnalysisResult analysisResult) {
		try {
		if (timeRange == "1D" && session == "all" && indexCounter == 5 ) {
			return analysisResult.getHighestCloseOpenStatus();
		} else {
			// save result in custom
			return analysisResult.getCustomIndicatorValue(getName() + "_HIGHESTCLOSEOPENTrend");			
		}
		}catch(Exception e) {
			return "HCOS DATA ERROR";
		}
	}

	public static int calculateSessionEndIndex(BarSeries series, int currentEndIndex, String session,
	...
	public static boolean checkForHigherPriceInLastNBars(BarSeries series,
			HighestCloseOpenIndicator.HighestValue highestVal, int indexCounter, int currentIndex) {
	...
	/**
	 * Helper method to get the highest close/open value with trend analysis using
	 * indexCounter
	 */
	public static HighestCloseOpenIndicator.HighestValue getHighestCloseOpenValue(BarSeries series, String timeRange,
			String session, String dataSource, int sessionEndIndex, int indexCounter, int currentAnalysisIndex) {

		// Create separate indicators for close and open
		HighestCloseOpenIndicator highestClose = new HighestCloseOpenIndicator(series, timeRange, true, dataSource,
				session, sessionEndIndex);
		HighestCloseOpenIndicator highestOpen = new HighestCloseOpenIndicator(series, timeRange, false, dataSource,
				session, sessionEndIndex);
		...
		if (highestCloseVal != null && highestOpenVal != null && highestCloseVal.value != null
				&& highestOpenVal.value != null) {
			...
			boolean hasHigherPrice = checkForHigherPriceInLastNBars(series, highestVal, indexCounter,
					currentAnalysisIndex);
			...
			// Determine final trend: Uptrend if ANY bar in last N exceeded the highest
			// value
			boolean isUptrend = hasHigherPrice || !isCurrentDowntrend;
	...
	protected void updateTrendStatus(AnalysisResult result, String currentTrend) {
		...
		if (timeRange == "1D" && session == "all" && indexCounter == 5 ) {
			...
			// save result in custom
			result.setCustomIndicatorValue(getName() + "_HIGHESTCLOSEOPENTrend", String.valueOf(currentTrend));
		}
	}
	...
	public String determineTrend(AnalysisResult result) {
		if (timeRange == "1D" && session == "all" && indexCounter == 5 ) {
			...
			// save result in custom
			return result.getCustomIndicatorValue(getName() + "_HIGHESTCLOSEOPENTrend");
		}
	}
	...
	public double calculateZscore(BarSeries series, AnalysisResult result, int endIndex) {
		...
		String status = determineTrend(result);//result.getHighestCloseOpenStatus();
