...
public class RSIStrategy extends AbstractIndicatorStrategy {
		...
		try {
			...
			if (endIndex >= period - 1) {
				...
				if (period == 14) {
					...
					result.setRsiZscore(zscore);
				} else {
					result.setCustomIndicatorValue(name + "_RSI", String.valueOf(rsiValue));
					result.setCustomIndicatorValue(name + "_RSITrend", String.valueOf(trend));
					result.setCustomIndicatorValue(name + "_RSIZscore", String.valueOf(zscore));
				}
				if (logger != null) {
					logger.log(String.format("RSI(%d) for %s: %.2f - %s", period, symbol, rsiValue, trend));
				}
			}
		} catch (Exception e) {
			if (logger != null) {
	...
	protected void updateTrendStatus(AnalysisResult result, String currentTrend) {
		if (period == 14) {
			...
			result.setCustomIndicatorValue(getName() + "_RSITrend", String.valueOf(currentTrend));
		}
	}
	...
	public String determineTrend(AnalysisResult result) {
		double rsiValue = 0;
		if (period == 14) {
			 rsiValue = result.getRsi();
		}else {
			rsiValue = Double.valueOf(result.getCustomIndicatorValue(getName() + "_RSI"));
		}	
		return determineRSITrend(rsiValue);
	}
	
	public double getRSIValue(AnalysisResult analysisResult) {
		 if(getPeriod() == 14) {
		        double rsiValue = analysisResult.getRsi();
		        return rsiValue;
	        }else {
	        	
	        	String rsiName = getName();
	        		        
		        double rsiValue = Double.valueOf(analysisResult.getCustomIndicatorValue(rsiName +"_RSI"));
		        return rsiValue;
	        }
	}

	public String getRSITrend(AnalysisResult analysisResult) {
		 if(getPeriod() == 14) {
		        String trend = analysisResult.getRsiTrend();		        
		        return trend;
	        }else {	        	
	        	String rsiName = getName();
		        String trend = (String) (analysisResult.getCustomIndicatorValue(rsiName +"_RSITrend"));		        
		        return trend;
	        }
	}
		...
		try {
			...
			if (logger != null) {
				logger.log(String.format(
						"RSI(%d) Z-score for %s: %.1f (Zone: %.1f, Momentum: %.1f, Trend: %.1f, Divergence: %.1f)",
						period, symbol, normalizedZscore, zoneScore, momentumScore, trendAlignmentScore,
						divergenceScore));
			}
	...
	private double calculateDivergenceScore(BarSeries series, RSIIndicator rsi, int endIndex) {
		...
		try {
			...
			for (int i = 1; i <= 5; i++) {
				...
				if (priceNow < pricePrev && pricePrev < priceOlder && rsiNow > rsiPrev && rsiPrev > rsiOlder) {
