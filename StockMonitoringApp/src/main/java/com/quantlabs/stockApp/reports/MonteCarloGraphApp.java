...
public class MonteCarloGraphApp {
    ...
    public MonteCarloGraphApp(List<String> symbols, String titleName) {
        initializeApp(symbols, titleName);
    }
    ...
    public MonteCarloGraphApp(List<String> symbols, String titleName, ZonedDateTime startTime, ZonedDateTime endTime, String timeframe) {
    ...
    public MonteCarloGraphApp(List<String> symbols, StockDataProviderFactory providerFactory, ConsoleLogger logger, String titleName) {
        initializeApp(symbols, providerFactory, logger, titleName);
    }

    public MonteCarloGraphApp(List<String> symbols, StockDataProviderFactory providerFactory, ConsoleLogger logger, String titleName, Set<String> topList) {
        this.topList = topList != null ? new HashSet<>(topList) : new HashSet<>();
        initializeApp(symbols, providerFactory, logger, titleName);
    }
    ...
    private void initializeApp(List<String> symbols, String titleName) {
        try {
            MonteCarloDataSourceManager dataSourceManager = createSimpleDataSourceManager();
            
            // Pass time range parameters to controller
            this.controller = new MonteCarloGraphController(symbols, dataSourceManager, titleName, topList,
                                                          initialUseCustomTimeRange, initialStartTime, initialEndTime, initialTimeframe);
            ...
            if (initialUseCustomTimeRange) {
                System.out.println("Custom time range: " + initialStartTime + " to " + initialEndTime + " timeframe: " + initialTimeframe);
            }
    ...
    private void initializeApp(List<String> symbols, StockDataProviderFactory providerFactory, ConsoleLogger logger, String titleName) {
        try {
            // Pass time range parameters to controller
    ...
    private void applyInitialTimeRange() {
        if (initialUseCustomTimeRange && initialStartTime != null && initialEndTime != null) {
            ...
            System.out.println("Applied initial custom time range: " + initialStartTime + " to " + initialEndTime + " with timeframe " + initialTimeframe);
        } else {
     ...
     * @param titleName 
     */
    public void initialize(List<String> symbols, String titleName) {
        if (!initialized) {
            initializeApp(symbols, titleName);
        }
    }
    
    public void initialize(List<String> symbols, String titleName, Set<String> topList) {
        this.topList = topList != null ? new HashSet<>(topList) : new HashSet<>();
        if (!initialized) {
            initializeApp(symbols, titleName);
        }
 ...
 // Add these methods to MonteCarloGraphApp.java
        ...
        SwingUtilities.invokeLater(() -> {
            try {
                ...
                javax.swing.JOptionPane.showMessageDialog(null, 
                    "Failed to initialize Monte Carlo Graph: " + e.getMessage(), 
    ...
    public List<String> addSymbols(Object symbolsInput) {
        if (!checkInitialization()) return new ArrayList<>();
     ...
     * @param symbolsInput Comma-separated string of symbols or List of symbols to remove
     */
    ...
    private boolean isSymbolValid(String symbol, MonteCarloDataSourceManager dataSourceManager) {
        ...
        try {
            ...
            boolean hasData = result.containsKey(cleanSymbol) && 
                            !result.get(cleanSymbol).isEmpty();
    ...
    private boolean isValidSymbolFormat(String symbol) {
        // Basic symbol format validation
        // Symbols should be 1-5 characters, alphanumeric, and may include dots or dashes
