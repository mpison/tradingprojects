...
public class AnalysisOrchestrator0 {
    
    private static TechnicalAnalysisService technicalAnalysisService;
    private static StockDataProvider dataProvider;
    
    public AnalysisOrchestrator0(TechnicalAnalysisService technicalAnalysisService, StockDataProvider dataProvider) {
        this.technicalAnalysisService = technicalAnalysisService;
        this.dataProvider = dataProvider;
    }
    
	public static LinkedHashMap<String, AnalysisResult> analyzeSymbol(String symbol, Set<String> timeframes, 
            Set<String> indicatorsToCalculate, boolean uptrendFilterEnabled,  Map<String, JComboBox<String>> indicatorTimeRangeCombos, Map<String, JComboBox<String>> indicatorSessionCombos, 
            Map<String, JTextField> indicatorIndexCounterFields, Map<String, Map<String, Integer>> timeframeIndexRanges, String currentDataSource) {
        
        LinkedHashMap<String, AnalysisResult> results = new LinkedHashMap<>();
        String previousTimeframeConsensusStatus = null;
        
        for (String timeframe : timeframes) {
            if (!indicatorsToCalculate.isEmpty()) {
	                try {
	                    ...
	                    AnalysisResult result = TechnicalAnalysisService.performTechnicalAnalysis(series, indicatorsToCalculate, timeframe, 
	                    		symbol, timeRange, session, indexCounter,
	                    		timeframeIndexRanges, currentDataSource, null);
	...
	public Map<String, String> analyzeCustomIndicators(String symbol, String timeframe, 
	    ...
	    try {
	        ...
	        for (CustomIndicator customIndicator : customIndicators) {
	            String result = calculator.calculateCustomIndicator(customIndicator, series, timeframe, symbol).toString();
	            customIndicatorResults.put(customIndicator.getName(), result);
	        }
	        
	    } catch (Exception e) {
	        //logToConsole("Error analyzing custom indicators for " + symbol + " " + timeframe + ": " + e.getMessage());
	    }
    ...
    private static boolean isBullishStatus(String status) {
        return status != null && (status.contains("Bullish") || status.contains("Uptrend") || 
               status.contains("↑") || status.contains("Buy"));
    }
	...
	public static void setTechnicalAnalysisService(TechnicalAnalysisService technicalAnalysisService) {
		AnalysisOrchestrator0.technicalAnalysisService = technicalAnalysisService;
	}
	...
	public static void setDataProvider(StockDataProvider dataProvider) {
		AnalysisOrchestrator0.dataProvider = dataProvider;
	}
